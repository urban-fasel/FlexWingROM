...
for k = 1:n_g
    if runTrainingPhase
        ...
        if k == n_g % load initial conditions from lowest velocity: that's the velocity at which the testing phase initiates
            training_restartIN = load(strcat(pwd,filesep,'data',filesep,paramFSI.wingParams.airfoil,filesep,'ROM',...
                filesep,'V',num2str(V0(1)),filesep,'training_restartIN.mat')); % load wing parameters
